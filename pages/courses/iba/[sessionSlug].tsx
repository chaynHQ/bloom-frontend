...
// TODO add padding here that does't come in from container
const containerStyle = {
  backgroundColor: 'secondary.light',
  paddingLeft: '20 !important',
  paddigRight: '20 !important',
  borderColor: 'red',
  borderStyle: 'solid',
} as const;
...
export async function getStaticProps({ locale, preview = false, params }: GetStaticPropsContext) {
  ...
  const sbParams = {
    resolve_relations: 'session_granular.course',
    version: preview ? 'draft' : 'published',
    language: locale,
    ...(preview && { cv: Date.now() }),
  };

  let { data } = await Storyblok.get(`cdn/stories/courses/iba/${sessionSlug}/`, sbParams);
  console.log('Session in IBA', data.story.content.bonus);
...
export async function getStaticPaths({ locales }: GetStaticPathsContext) {
  let { data } = await Storyblok.get('cdn/links/?starts_with=courses/iba');
