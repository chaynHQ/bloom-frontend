...
- `POST /components` for:
  ...
  - `grounding_page` — the `/grounding` landing page (hero + SEO), separate from `resource_grounding` (the exercises). Standard page fields only (`title`, `seo_description`, `description`, `header_image`), cloned from `page`. Added later via `scripts/storyblok/01b-create-grounding-page.mjs` (id 218206054215728, created 2026-09-09); step 6's `04` moves the flat `grounding` story onto it. Rendered by `components/storyblok/StoryblokGrounding.tsx`.
...
5. **Grounding page is now a real Storyblok story.** `01b-create-grounding-page.mjs` created a `grounding_page` component (id 218206054215728) — the space's standard page fields (`title`, `seo_description`, `description` richtext, `header_image`), cloned from `page`, no `page_sections`. Frontend: `components/storyblok/StoryblokGrounding.tsx` (replaces `components/pages/GroundingPage.tsx`), registered as `grounding_page` in `lib/storyblok.ts`; `app/[locale]/grounding/page.tsx` fetches `grounding/overview` and **falls back to the flat `grounding` story** until step 6's `04` moves it — so this ships in the redesign deploy and works before and after the move. `04-delete-flat-pages.mjs` converts the flat `grounding` story (keeps uuid/id) into `grounding/overview` on the `grounding_page` component. Structure decided: `grounding/` folder + page story **inside** it at `overview`.
...
- **Post-verify, only after production is confirmed green:**
  - `scripts/storyblok/04-delete-flat-pages.mjs` — (1) `DELETE` the flat `activities` story (339097530; `component: page`, no backend row, shadowed by the `/activities`→library redirect); (2) **convert** the flat `grounding` story (337379594): move into the grounding folder, reslug `overview`, set `content.component = grounding_page`, drop `page_sections` (+ i18n copies), keep the hero fields, re-publish — the story keeps its uuid/id; (3) rename the folder slug `grounding-exercises` → `grounding` (so the page is `grounding/overview`) and add `grounding_page` to the folder `content_types`. Rename is transparent: grounding route + library select by `component` (never folder path), `related_grounding` refs are uuid-based, leaf `?id=` slugs unchanged, backend ignores `resource_grounding`. Guards on `is_folder` / `component` / `grounding_page` existing; snapshots both stories' full content before writing. Child re-publish opt-in (`--republish-children`). Dry-run 2026-09-09 clean: delete `activities` → convert `grounding` → rename folder.
...
- **Grounding page content lives in Storyblok** — `StoryblokGrounding` renders its hero from the `grounding_page` story (`grounding/overview`), with a fallback to the flat `grounding` story during the transition. `04-delete-flat-pages.mjs` converts the flat story in place (uuid/id kept). Don't `DELETE` the flat `grounding` story — it's converted, not deleted.
