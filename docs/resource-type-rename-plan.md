...
**Done** (branch `resource-types-step-7a`). **The dual-path / fallback-fetch scaffolding was dropped** — a ~5-minute content gap between the 7a deploy and 7c is accepted instead. So there is **no 7d "revert scaffolding" step**; the leftovers fold into step 8.

What shipped, vs the plan text below:

- **No transitional fetch.** `app/[locale]/video/[slug]/page.tsx` (new) and `audio/[slug]/page.tsx` (already existed from step 5, now unchanged from its clean form) mirror the `activity`/`written` routes exactly: a single `getStoryblokStory('video/${slug}' | 'audio/${slug}')`, `resolve_relations` for `resource_video.*` / `resource_audio.*` only, `dynamicParams = false`, `generateStaticParams` from `resourceFolderStaticParams`. `getOptionalStoryblokStory` is deleted (it existed only for the fallback).
  - **Consequence:** between the 7a prod deploy and 7c, `/video/*` + `/audio/*` 404 (and the old→new 301s land on that 404), and the library shows no video/audio cards. After 7c moves content, a **prod rebuild** is required to regenerate `generateStaticParams`. This is the accepted gap.
- **Media events cleaned up globally.** `<Video>` / `<ResourceAudioPlayer>` now append just `_STARTED` / `_PLAYED` / `_PAUSED` / `_FINISHED`; every caller passes the full family prefix (`SESSION_VIDEO`, `COURSE_INTRO_VIDEO`, `STORYBLOK_VIDEO`, `STORYBLOK_AUDIO`, and `RESOURCE_VIDEO` / `RESOURCE_AUDIO` from the merged pages). Net-zero for the session / course-intro / storyblok-video event names; `StoryblokAudio` (`STORYBLOK_AUDIO_PLAYER_AUDIO_*` → `STORYBLOK_AUDIO_*`) and the session-chat intro video (`SESSION_CHAT_VIDEO_VIDEO_*` → `SESSION_CHAT_VIDEO_*`) shed an accidental doubled word. Resource media events become the clean `RESOURCE_VIDEO_STARTED` / `RESOURCE_AUDIO_STARTED`; `events.ts` constants updated to match, conversation constants deleted, old→new comment map dropped in favour of the shorter header comment.
- `RESOURCE_CATEGORIES` is now `{ VIDEO, AUDIO, WRITTEN, ACTIVITY }`; the enum-value drift (`CONVERSATION = 'resource_conversation'`) is resolved — GA `resource_category` is `video` / `audio` henceforth, including for a story still on the old component.
- `StoryblokResourceConversationPage` + its route deleted; `StoryblokResourceAudioPage` (step 5) takes over with `loginRequiredByDefault: true`. The merged `StoryblokResourceVideoPage` (short + single) also sets `loginRequiredByDefault: true` — fail-safe; 7c sets an explicit `login_required: false` on the public shorts as it moves them.
- `ResourceCarousel` + `libraryData.FORMAT_BY_COMPONENT` keep the old `resource_short_video` / `resource_single_video` / `resource_conversation` keys (mapped to the `video` / `audio` category) so hand-picked `resource_carousel` refs and `related_content` pointing at a not-yet-moved story still render a card during 7c. Removed in step 8.
- `getLibraryStories` queries `video/` + `audio/` (+ `written/` `activity/`) only — the old folders and the `somatics` tag filter are gone.
- Redirects (permanent, bare + `/${LOCALE_PATTERN}/`): `/shorts/:slug` + `/videos/:slug` → `/video/:slug`, `/conversations/:slug` → `/audio/:slug`. No per-slug renames (the 2026-09-09 scan found none — **re-run it at 7c**; any renames go above the folder-swap rules).
- Old routes + `StoryblokResourceShortPage` / `StoryblokResourceSingleVideoPage` / `StoryblokResourceConversationPage` + `shorts.cy` deleted; `videos.cy` → `video.cy`, `conversations.cy` → `audio.cy` (both **red until 7c runs on the Cypress Storyblok space** — they visit `/video/*` `/audio/*`).
- i18n: `Resources.{shorts,videos,conversations}` → `Resources.video` (`Resources.audio` was already there from step 5); `relatedContent.video` added. Dead old `relatedContent` keys (`short_video`, `single_video`, `resource_conversation`, `activities`) left for step 8. `nextConversationButtonLabel` / `conversationTranscriptLink` — n/a (steps 4–5 removed those features).
- `[slug]/page.tsx` `excludePaths` and `scripts/translateStoryblok.mjs` exclude sets updated for `video` / `audio`.
- **Not touched:** `library.cy` (its `somatics` search still resolves post-7c — the story keeps its title), `public/sitemap.xml` (regenerate in step 8), `STORYBLOK_TAGS` (now dead — step 8), `libraryData.AUTHENTICATED_PATH_HEADS` (dead post-7c — step 8).
