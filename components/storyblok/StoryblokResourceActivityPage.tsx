...
const EVENT_PREFIX = 'RESOURCE_ACTIVITY' as const;

const StoryblokResourceActivityPage = ({ story: initialStory }: { story: ISbStoryData }) => {
  ...
  } = story.content as StoryblokResourceActivityPageProps;
  ...
  const eventData = useMemo(
    () => ({
      resource_category: RESOURCE_CATEGORIES.ACTIVITY,
      resource_name: name,
  ...
  useEffect(() => {
    logEvent(RESOURCE_ACTIVITY_VIEWED, eventData);
  });
  ...
  // Activities have no play button to hook "started" to — reading the page is the
